...
export const DoctorWorkstation: React.FC<DoctorWorkstationProps> = ({
  ...
  const handleRagSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/gemini/rag-query', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error(e);
      setRagResponse({ answer: 'Failed to reach RAG server.', sources: [] });
    } finally {
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-emerald-950 text-white rounded-2xl p-5 shadow-sm border border-slate-700">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              onClick={() => setActiveSubTab('clinical_rag_assistant')}
              className={`px-3.5 py-2 rounded-xl text-xs font-semibold transition-colors cursor-pointer flex items-center gap-1.5 ${
                activeSubTab === 'clinical_rag_assistant'
...
District: ${gisContext.city}, ${gisContext.state} | AQI: ${gisContext.aqi} | Active Outbreaks: ${gisContext.activeOutbreaks.map((o) => o.disease).join(', ') || 'None'}`;
      ...
      {/* Sub-Tab 4: Clinical RAG Assistant */}
      {activeSubTab === 'clinical_rag_assistant' && (
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs space-y-4">
          <div className="border-b border-slate-100 pb-3">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Ask medical guidelines, treatment protocols, and cross-reference symptoms against standard medical knowledge bases.
            </p>
          </div>
          
          <div className="space-y-4">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 min-h-[150px] text-sm text-slate-700 flex flex-col justify-end">
              {ragResponse ? (
                <div className="space-y-2 mt-4">
                  ...
                  {ragResponse.confidence && (
                    <div className="text-xs text-purple-600">
                      <strong>Confidence:</strong> {Math.round(ragResponse.confidence * 100)}%
                    </div>
                  )}
                </div>
              ) : (
                <p className="text-center text-slate-400 italic">I am ready to query the medical guidelines. Ask me anything.</p>
              )}
            </div>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="e.g. What is the standard protocol for acute chest pain with diaphoresis?"
                className="flex-1 bg-white border border-slate-300 rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
              <button 
                onClick={handleRagSubmit}
                disabled={isRagLoading || !ragQuery.trim()}
                className="bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white px-4 py-2 rounded-xl text-sm font-bold transition-colors shadow-sm flex items-center gap-2 cursor-pointer"
              >
                {isRagLoading ? (
                ...
                {isRagLoading ? 'Querying...' : 'Query RAG'}
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Sub-Tab 5: Official ABDM Print / PDF Sheet */}
      {activeSubTab === 'ehr_print' && (
