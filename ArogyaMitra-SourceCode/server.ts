...
8. educationalDisclaimer: Standard legal and clinical disclaimer`;
  ...
  // API 8: RAG System for Clinical Guidelines & Knowledge Retrieval
  app.post('/api/gemini/rag-query', async (req, res) => {
    const { query } = req.body;
    const ai = getGeminiClient();

    // 1. Retrieval Phase: Mock vector DB / document extraction
    // In a real system, this would embed the query and search a vector DB.
    // For this implementation, we construct the context dynamically from our catalog
    // and standard medical knowledge.
    const retrievedContext = `
      [DOCUMENT 1: TRIAGE RED FLAGS]
      Critical conditions requiring immediate 108 ambulance:
      - SpO2 < 90% or Pulse > 130 bpm
      - Systolic BP > 180 mmHg or Diastolic BP > 110 mmHg (Hypertensive Crisis)
      - Systolic BP < 85 mmHg (Shock)
      - Chest Pain with left arm radiation
      - High Fever with stiff neck
      
      [DOCUMENT 2: AYUSH GUIDELINES]
      Vata imbalance causes joint pain and insomnia.
      Pitta imbalance causes acidity and skin rashes.
      Kapha imbalance causes congestion and lethargy.
      
      [DOCUMENT 3: DENGUE PROTOCOL]
      Retro-orbital pain (pain behind eyes) with fever is a classic Dengue indicator. Check platelet counts.
    `;

    const fallbackResponse = {
      answer: "Based on the clinical guidelines, " + query + " requires further medical evaluation. (Fallback activated due to rate limits)",
      sources: ["Mock DB"],
      confidence: 0.7
    };

    try {
      const prompt = `You are a clinical RAG (Retrieval-Augmented Generation) assistant.
Use ONLY the provided context documents to answer the user's query. If the answer is not in the context, say "I don't have enough information based on the guidelines."
...
Return JSON with:
{
  "answer": "Detailed answer based strictly on context",
  "sources": ["List of document names used"],
  "confidence": 0.95
}
`;

      const result = await generateGeminiSafe<any>(
        ai,
        {
          contents: prompt,
          config: {
